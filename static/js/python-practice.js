...
(() => {
  ...
  function quizQuestion(q,selected,disabled=false,review=null){return `<section class="quiz-question" id="question-${q.id}" data-question="${q.id}"><span class="eyebrow">${escape(q.kind)}</span><h3>${escape(q.question)}</h3><fieldset class="options"><legend class="sr-only">${escape(q.question)}</legend>${q.options.map((option,index)=>{const correct=review&&index===review.answer,wrong=review&&!review.correct&&index===review.selected;const state=correct?' correct-answer':wrong?' wrong-answer':'';return `<label class="option${state}"><input type="radio" name="${q.id}" value="${index}" ${selected===index?'checked':''} ${disabled?'disabled':''}><span>${escape(option)}</span></label>`;}).join('')}</fieldset><div class="feedback"></div></section>`;}
  ...
  function reviewQuiz(result){
    quizContext=null;
    show(`<div class="quiz-sheet">${heading(escape(moduleOf(result.module).title),'Review each answer and its explanation. This result is saved in your progress.','QUIZ REVIEW')}<div class="exam-summary"><div class="score-ring">${result.score}/${result.total}</div><div><h2>Checkpoint complete</h2><p>${result.score===result.total?'All answers correct.':'Use the explanations below to work through your mistakes.'}</p></div></div>${result.details.map(d=>{const q=data.quizzes.find(q=>q.id===d.id);return `${quizQuestion(q,d.selected,true,d)}<div class="answer-feedback ${d.correct?'is-correct':'is-wrong'}"><strong>${d.correct?'✓ Correct':'✕ Incorrect'}</strong>${d.correct?'':`<p class="correct-answer-text">Correct answer: ${escape(q.options[d.answer])}</p>`}<p>${escape(d.explanation)}</p></div>`;}).join('')}<div class="quiz-actions"><button id="quiz-done">Back to quizzes</button><button id="quiz-practice">Practice this topic →</button>${result.score<result.total?'<button id="quiz-retake" class="primary">Retake full quiz →</button><button id="quiz-missed">Retry missed questions →</button>':'<button id="quiz-again">Take full quiz again</button>'}</div></div>`);
    $('#quiz-done').onclick=()=>navigate('quiz');$('#quiz-again')?.addEventListener('click',()=>openQuiz(result.module,'all'));$('#quiz-retake')?.addEventListener('click',()=>openQuiz(result.module,'all'));
